#=
    Arguments:
        ...
        -data to only build the headers and assets that normally go into exe mode
=#
...
if ("-exe" in ARGS) + ("-dll" in ARGS) + ("-data" in ARGS) != 1
    error("Must give exactly one of [ '-exe', '-dll', '-data' ]")
end
...
per_mode(if_exe, if_dll, if_data) = if MODE == :exe
    if_exe
elseif MODE == :dll
    if_dll
elseif MODE == :data
...
const OUTPUT_FOLDER_NAME = per_mode("build-exe", "build-dll", "build-exe")
...
const COPY_DEST = per_mode(joinpath(OUTPUT_DIR, "bin"), joinpath(OUTPUT_DIR, "gui_tool_files"), joinpath(OUTPUT_DIR, "bin"))
mkpath(COPY_DEST)
...
open(joinpath(INCLUDE_DIR, "$CONSTS_FILE_NAME.h"), "w") do file
    ...
    //This file was generated by the MarkovJunior.jl compiler script.
    ...
    #define JMJ_TICK_PRIORITY_MINIMAL $(MJ.STANDARD_MIN_COMPILE_TIME_TICK_PRIORITY-1)
    #define JMJ_TICK_PRIORITY_LOW $(MJ.STANDARD_MIN_COMPILE_TIME_TICK_PRIORITY)
    #define JMJ_TICK_PRIORITY_HIGH $(MJ.STANDARD_END_OF_OP_TICK_PRIORITY)
...
open(joinpath(INCLUDE_DIR, "$CONSTS_FILE_NAME.hpp"), "w") do file
    print(file, """
    #ifndef $(HEADER_GUARD_NAME_1)_PP
    #define $(HEADER_GUARD_NAME_1)_PP
    ...
    namespace jmj {
        ...
        constexpr size_t TickPriorityMinimal = $(MJ.STANDARD_MIN_COMPILE_TIME_TICK_PRIORITY-1);
        constexpr size_t TickPriorityLow = $(MJ.STANDARD_MIN_COMPILE_TIME_TICK_PRIORITY);
        constexpr size_t TickPriorityHigh = $(MJ.STANDARD_END_OF_OP_TICK_PRIORITY);
    }
...
open(joinpath(INCLUDE_DIR, "$CONSTS_FILE_NAME.json"), "w") do file
    print(file, """{
        ...
        "tick_priority_minimal": $(MJ.STANDARD_MIN_COMPILE_TIME_TICK_PRIORITY-1),
        "tick_priority_low": $(MJ.STANDARD_MIN_COMPILE_TIME_TICK_PRIORITY),
        "tick_priority_high": $(MJ.STANDARD_END_OF_OP_TICK_PRIORITY)
    }""")
end
...
if MODE in (:exe, :data)
    ...
    open(joinpath(INCLUDE_DIR, "jmj_ipc.h"), "w") do file
        ...
        #define JMJ_IPC_DEFAULT_MAX_GRID_DIMS $(MJ.IPC_DEFAULT_SAFETY_CAPS.max_grid_dimensions)
        #define JMJ_IPC_DEFAULT_MAX_ALGO_SRC_LEN $(MJ.IPC_DEFAULT_SAFETY_CAPS.max_algo_string_len)
    ...
    open(joinpath(INCLUDE_DIR, "jmj_ipc.hpp"), "w") do file
        print(file, """
        #ifndef $(HEADER_GUARD_NAME_2)_PP
        #define $(HEADER_GUARD_NAME_2)_PP
        ...
        namespace jmj { namespace ipc {
            ...
            constexpr size_t DefaultMaxGridDims = $(MJ.IPC_DEFAULT_SAFETY_CAPS.max_grid_dimensions);
            constexpr size_t DefaultMaxAlgoSrcLen = $(MJ.IPC_DEFAULT_SAFETY_CAPS.max_algo_string_len);
    ...
    open(joinpath(INCLUDE_DIR, "jmj_ipc.json"), "w") do file
        print(file, """{
            ...
            "default_max_grid_dims": $(MJ.IPC_DEFAULT_SAFETY_CAPS.max_grid_dimensions),
            "default_max_algo_src_len": $(MJ.IPC_DEFAULT_SAFETY_CAPS.max_algo_string_len),
